...
type RefundInfoType FundInfoType

func (t TransferStruct) Refund(data RefundInfoType) (*models.Transfer, error) {
	if data.Amount <= 0 {
	...
	err := config.DB.Transaction(func(tx *gorm.DB) error {
		...
		if agent.Balance < float64(data.Amount) {
			return fmt.Errorf("agent does not have enough balance to transfer")
		}

		// 2. Create transaction record
		tfRecord := models.Transfer{
			Type:    models.TransferTypeRefund,
			Amount:  data.Amount,
			AgentID: agent.ID,
